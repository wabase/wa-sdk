...
function validateConfig() {
  const required = ['accessToken', 'phoneNumberId', 'wabaId', 'businessId', 'testRecipient'];
...
async function testMessagesAPI() {
  ...
  await runTest('Messages', 'Send Video (URL)', async () => {
    await client.messages.sendVideo({
      to: config.testRecipient,
      video: { link: config.videoUrl || 'https://file-examples.com/wp-content/storage/2017/04/file_example_MP4_480_1_5MG.mp4' },
      caption: 'Test Video',
    });
  }, !config.videoUrl && !process.env.TEST_VIDEO_URL);

  await runTest('Messages', 'Send Audio (URL)', async () => {
    await client.messages.sendAudio({
      to: config.testRecipient,
      audio: { link: config.audioUrl || 'https://file-examples.com/wp-content/storage/2017/11/file_example_MP3_700KB.mp3' },
    });
  }, !config.audioUrl && !process.env.TEST_AUDIO_URL);

  await runTest('Messages', 'Send Document (URL)', async () => {
    await client.messages.sendDocument({
      to: config.testRecipient,
      document: { link: config.documentUrl || 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf' },
...
async function testAccountAPI() {
  ...
  await runTest('Account', 'Get Business Profile (Custom Fields)', async () => {
    await client.account.getBusinessProfile(['about', 'address', 'description', 'email']);
  });
  ...
  await runTest('Account', 'Get Phone Number (Alternative Method)', async () => {
  ...
  await runTest('Account', 'Get Conversational Automation', async () => {
    ...
    console.log(`   Ice breakers: ${config_data.data?.[0]?.ice_breakers?.length || 0}`);
  });
...
async function testFlowsAPI() {
  ...
  await runTest('Flows', 'Get Flow Details', async () => {
    if (createdFlowId) {
  ...
  await runTest('Flows', 'Update Flow', async () => {
    if (createdFlowId) {
  ...
  await runTest('Flows', 'Get Flow Preview', async () => {
    if (createdFlowId) {
      await client.flows.getPreview(createdFlowId);
    }
  }, !createdFlowId);

  await runTest('Flows', 'List Flow Assets', async () => {
    if (createdFlowId) {
      await client.flows.listAssets(createdFlowId);
    }
  }, !createdFlowId);

  await runTest('Flows', 'Publish Flow', async () => {
    if (createdFlowId) {
      await client.flows.publish(createdFlowId);
    }
  }, true); // Always skip - flow needs content before publish

  await runTest('Flows', 'Delete Flow', async () => {
    if (createdFlowId) {
...
async function testMediaAPI() {
  ...
  await runTest('Media', 'Get Media URL', async () => {
    if (uploadedMediaId) {
      const urlInfo = await client.media.getUrl(uploadedMediaId);
      console.log(`   Media URL obtained: ${urlInfo.url.substring(0, 50)}...`);
    }
  }, !uploadedMediaId);

  await runTest('Media', 'Download Media', async () => {
    if (uploadedMediaId) {
      const mediaData = await client.media.download(uploadedMediaId);
      const dataLength = mediaData.data ? 
        (Buffer.isBuffer(mediaData.data) ? mediaData.data.length : mediaData.data.byteLength) : 
        'unknown';
...
async function testEmbeddedSignupAPI() {
  ...
  await runTest('Embedded Signup', 'Get Message Templates (via Templates API)', async () => {
...
async function testWebhooksAPI() {
  ...
  await runTest('Webhooks', 'Verify Webhook Signature', async () => {
    ...
    console.log(`   Signature validation: ${isValid ? 'Valid' : 'Invalid (expected)'}`);
  });
